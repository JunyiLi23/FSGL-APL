...
class Experiment:
    def __init__(self, data, api_key, data_path) -> None:
        self.raw_texts = data.raw_texts               #初始文本 - 节点自身信息
    ...
    def async_api(self, prompts, label_names, temperature = None, top_p = None, n = 1, dataset = 'cora', key = 'zero_shot', rewrite = False, fix = False, seed = 0, sp = 2, ss = 2):
        ...
        fixed_grammar = 0
        g_error_idx = []
        cache = self.load_cache(dataset, key)                                       #[]加载查询openai的结果文件，若无则返回none
        ## cache for openai result
        if cache != None:
            cache = cache.get(key, None)
        ## cache for extracted annotation result, may present inconsistency with openai result
        result_cache = self.load_saved(dataset, key)                                #[]加载注释结果文件
        result_cache_pred = result_cache['pred']
        # import ipdb; ipdb.set_trace()
        valid_number = len([x for x in prompts if x != ""])                         #筛选节点的个数
        input_filename = "prompt_async_input_{}_{}_temperature_{}_n_{}_input_seed_{}_{}.json".format(dataset, key, temperature, n, seed, valid_number)    #查询大模型的输入文件
        output_filename = "prompt_async_input_{}_{}_temperature_{}_n_{}_output_seed_{}_{}.json".format(dataset, key, temperature, n, seed, valid_number)  #查询大模型的输出文件
        # import ipdb; ipdb.set_trace()
        cache_hit = 0
        result_cache_hit = 0
        for i in range(len(prompts)):
            if cache != None and cache[i] != "" and isinstance(cache[i], list) and prompts[i] != "" and not rewrite:   #若已存在非空的LLM查询结果
                check_res, error_type, error_message = self.check_grammar(cache[i][0])
                if check_res:
                    prompts[i] = ""
                    cache_hit += 1                                                  #缓存结果（openai查询）命中次数加一
            else:
                try:
        ...
        if result_cache_hit == len(select_ids):
            print("All cache hit, no need to run the annotation")                   #注释结果文件全部命中
            sys.exit()
        openai_result = efficient_openai_text_api(prompts, input_filename, output_filename, sp, ss, api_key=self.api_key, temperature=temperature, n = n, rewrite = rewrite)
        # import ipdb; ipdb.set_trace()
        ...
        #     time.sleep(60)
        for i, res in tqdm(enumerate(openai_result)):
            if i not in select_ids:                                                                                  #未选择的节点，直接继续循环
                responses.append("")
                continue
            # if key != 'consistency':
            if (cache != None and cache[i] != "" and res[0] == "") or (result_cache_pred[i] != -1 and res[0] == ""): #缓存结果存在且新查询结果为空，注释结果存在且新查询结果为空
                responses.append(cache[i])
                continue
            # else:
...
".format(question, k)  #.format()允许用户以一种简洁而灵活的方式将变量插入到字符串中，同时可以控制显示的格式、对齐方式和精度等。这里使用{}占位符来插入变量
    ...
    def retrieve_answer(self, answer, data):
        ...
        for result in answer:
            ...
            if result == "":   #大模型生成结果为空
                ...
                output.append(res)
                continue
            line = result[0].lower() #将字符串中的所有大写字母转换为小写字母
            try:
                this_dict = retrieve_dict(line) #字符串类型转换
                res = []
                for dic in this_dict:
                    answer = dic['answer']
                    confidence = dic['confidence']
                    ...
                    res.append((answer, confidence))   #将大模型对各筛选节点的预测文本解析为变量（类预测，置信度分数）
                output.append(res)
            except:
                answer = get_closest_label(line, self.label_names)  #若输出结果line有乱码，则分别找类预测和置信度分数
                confidence = min(int(''.join(filter(str.isdigit, line))), 100)   #str.isdigit判断是否含有数字，filter按条件过滤，join拼接字符串
                # res = [("", 0)]
...
Old one: {} \n New one here:".format(old)
    ...
    def eval_result(self, res, label_names, select_ids, gt, dataset_name = 'cora', method='zero_shot', strategy = 'random', data_obj = None, seed = 0, g_error_idx = None):
        ...
        for i, r in enumerate(res):
            if i not in select_ids: continue  #非选择节点，跳过
            # import ipdb; ipdb.set_trace()
            # debug.append(i)
            voting = []
            if 'consistency' in method or 'all' in method:
                ...
                this_conf = []
                selected = False
                for selection in r:  #若是top-k提示策略，则这里的r包含k组预测结果
                    # import ipdb; ipdb.set_trace()
                ...
                if not selected:
                    ...
                    c = selection[0][1] / 2  #预测类非法，还找个最相似的类，置信度减半
                    p = label_names.index(p)
                    pred.append(p)
                    conf.append(c)
                    continue
                p = most_common_number(this_pred)  #top-k中选择出现次数最多的，即一致性提示策略
                first_appear = this_pred.index(p)  #对应的置信度取其首次出现时的
                base_c = 0
                ...
                for pp, cc in zip(this_pred, this_conf):
                    if pp == p:
                        ...
                        base_c += (100 - cc)
                base_c /= k
                pred.append(p)      #llm对节点的最终预测类
                conf.append(base_c) #llm对节点的最终预测的置信度分数
                # import ipdb; ipdb.set_trace()
            else:
                ans = r[0]
                p = ans[0]
        ...
        ## 1. all acc
        all_acc = (pred == gt_y).float().mean()  #准确率计算
        ...
        self.save_result(len(gt), select_ids, pred, conf, data_obj, dataset_name, seed, strategy, method) #保存llm预测结果，未选择节点对应内容为空
        total_num = len(gt)
        self.save_saved(dataset_name, method, pred, conf, select_ids, total_num) #？有必要，再检查和保存
...
def few_shot_prompts(texts, label_names, example, example_label, object_cat = "Paper", question = "Which arxiv cs subcategories does this paper belong to?", answer_format = "Give 3 likely arXiv CS sub-categories as a comma-separated list ordered from most to least likely together with a confidence ranging from 0 to 100, in the form of a list python dicts like [{\"answer:\":<answer_here>, \"confidence\": <confidence_here>}]"):
    ...
    for text in texts:
        ...
        prompt += zero_shot_prompt([example], label_names, need_tasks = True, object_cat = object_cat, question = question, answer_format = answer_format)[0]  #提供一些例子example，形式为zero-shot模板
        prompt += "\nOutput:\n[{{\"answer\":\"{}\", \"confidence\":{}}}]\n".format(example_label, 100)
        prompt += zero_shot_prompt([text], label_names, need_tasks = True, object_cat = object_cat, question = question, answer_format = answer_format)[0]   #本节点text的提示
        prompt += "\nOutput:\n"
        prompts.append(prompt)
    return prompts
...
def calculate_cost_from_a_list_of_texts(texts, select_ids = None):
    ...
    for i, text in enumerate(texts):
        if select_ids != None and i not in select_ids: continue  #选择节点集非空，且i不属于选择节点集
        if len(text) > 0:
            cost += num_tokens_from_string(text, model = 'gpt-3.5-turbo-0301')   #计算token数量。编码是将文本字符串拆解为token列表，GPT以token的形式来阅读文本，了解文本中的token数量，既可以告诉你字符串是否太长而超出了模型处理能力，也可以查看OpenAI的API费用（按照token计费）
    return cost
...
def consistency(topk_prompt, dataname, exp, temperature = 1.2, data = None, strategy = 'random', seed = 0, key_name = 'consistency'):
    ## 
    n = 3
    topk_response, error_idx = exp.async_api(topk_prompt, data.label_names, temperature, None, n = n, dataset = dataname, key = key_name, rewrite = False, fix = True, seed = seed, sp = 60, ss = 1.5) #查询大模型生成预测和置信度
...
if __name__ == '__main__':
    ...
    args = get_command_line_args_datasets()                                                               #参数设置
    params_dict = load_yaml(args.yaml_path)                                                               #路径、密钥
    ...
    full_mapping = load_mapping()                                                                         #类别与缩写的映射
    # total_budget = 150
    ...
    for dataname in need_datasets:
        ...
        object_cat = configs[dataname]['zero-shot']['object-cat']                    #configs.py存了所提四种prompt模板
        question = configs[dataname]['zero-shot']['question']
        answer_format = configs[dataname]['zero-shot']['answer-format']
        ...
        for seed in seeds:
            # import ipdb; ipdb.set_trace()
            select_samples = data.train_masks[seed]                               #通过了主动选择策略的无标签节点？
            select_ids = idxs[select_samples]                                     #对应id索引
            # import ipdb; ipdb.set_trace()
            orig_question = data.raw_texts                                        #各节点的原始文本
            ...
            questions = zero_shot_prompt(orig_question, data.label_names, True, object_cat, question, answer_format)    #【一】无样本提示，（其它提示根据此再做其他改变） —————————————————————————————————————
            no_task_question = zero_shot_prompt(orig_question, data.label_names, need_tasks=False, object_cat = object_cat, question=question, answer_format=answer_format) #此行 - 无任务即只生成预测，上一行 - 有任务是生成预测和置信度分数并按top-k排序
            ...
            if 'zero_shot' in exps:
                print("zero shot all")
                vanilla_prompt = [exp.vanilla_verbalized_confidence(q) for q in questions]       #各节点的提示问题
                prompt_cost = calculate_cost_from_a_list_of_texts(vanilla_prompt, select_ids)    #计算文本拆分token数量
                vanilla_prompt = pack_prompt(vanilla_prompt, select_ids, data.x.shape[0])        #将非选择节点的问题置空
                vanilla_response, error_idx = exp.async_api(vanilla_prompt, data.label_names, temperature = 0, top_p = None, n = 1, dataset = dataname, key = 'zero_shot', rewrite = False, fix = True, seed = seed, sp = 60, ss = 0) #查询大模型生成预测和置信度
                exp.save_cache(dataname, 'zero_shot', vanilla_response)
                res = exp.retrieve_answer(vanilla_response, data)                                #解析llm输出为（类，置信度）
                eval_out, _, _ = exp.eval_result(res, data.label_names, select_ids, data.y, dataname, method='zero_shot', strategy = args.strategy, data_obj = data, seed = seed, g_error_idx = error_idx)  #计算指标，评估大模型预测效果
                performance[dataname]['zero_shot'].append(eval_out)                              #[]性能 - 准确率
                response_str = [x[0] for x in vanilla_response if x != '']
                all_costs = prompt_cost + calculate_cost_from_a_list_of_texts(response_str) #对llm输出文本做编码后的token数
                cost[dataname]['zero_shot'].append(all_costs)                                    #[]消耗 - OpenAI的API费用（按照token计费）
            ...
            if 'few_shot' in exps:
                print("few shot")
                few_shot_prompt = few_shot_prompts(orig_question, data.label_names, example_text, example_labels, object_cat, question, answer_format) #少样本提示，即先给几个例子的提示，再进行本节点的提示
            ...
            if 'consistency' in exps:
                print("consistency")
                topk_step_prompt = [exp.topk_confidence(q, 3, dataname) for q in no_task_question]  #构建提示 - top-k策略
                topk_step_prompt = pack_prompt(topk_step_prompt, select_ids, data.x.shape[0])
                prompt_cost = calculate_cost_from_a_list_of_texts(topk_step_prompt, select_ids)
        ...
        if 'zero_shot' in exps:
            ...
            print(f"Zero-shot Test Accuracy: {mean_test_acc:.2f} ± {std_test_acc:.2f}")   #准确率 - 平均acc、标准差acc
            print(f"Zero-shot Average Cost: {avg_cost:.2f}")                              #消耗 - token数
