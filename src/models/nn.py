...
class SAGE(torch.nn.Module):
    def __init__(self, num_layers, input_dim, hidden_dimension, num_classes, dropout, norm=None) -> None:
        ...
        if num_layers == 1:
            ...
            #                            normalize=True))  #SAGEConv文档里没有参数cached？
            self.convs.append(SAGEConv(input_dim, num_classes, normalize=True))
        else:
            # self.convs.append(SAGEConv(input_dim, hidden_dimension, cached=False,
            #                            normalize=True))
            self.convs.append(SAGEConv(input_dim, hidden_dimension, normalize=True))
            if norm:
                self.norms.append(torch.nn.BatchNorm1d(hidden_dimension))
            else:
                self.norms.append(torch.nn.Identity())
            ...
            for _ in range(num_layers - 2):
                ...
                #              normalize=True))
                self.convs.append(SAGEConv(hidden_dimension, hidden_dimension, normalize=True))
                self.norms.append(torch.nn.BatchNorm1d(hidden_dimension))

            # self.convs.append(SAGEConv(hidden_dimension, num_classes, cached=False, normalize=True))
            self.convs.append(SAGEConv(hidden_dimension, num_classes, normalize=True))
...
class LP(torch.nn.Module):
    def __init__(self, num_layers, alpha) -> None:
        super().__init__()
        self.lp = LabelPropagation(num_layers, alpha)  #标签传播，局部社区划分算法，每次迭代中更改节点标签为其邻居标签中最多的那一类
