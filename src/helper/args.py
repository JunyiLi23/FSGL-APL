...
def get_command_line_args():
     ...
     parser.add_argument('--dataset', default='', type=str)              #【数据集】：cora、pubmed、citeseer、arxiv
     parser.add_argument('--normalize', default=0, type=int)
     parser.add_argument('--epochs', type=int, default=300)              #【训练轮次】，cora 20、citeseer 16
     ...
     parser.add_argument('--main_seed_num', type=int, default=5)         #【随机种子】，cora 3、citeseer 5
     ...
     parser.add_argument('--early_stop_start', type=int, default=400)               #【早停参数，防止过拟合】cora 150
     # parser.add_argument('--alpha', type=float, default=0.9)
     parser.add_argument('--low_label_test', type=int, default=0)
     parser.add_argument('--few_shot_test', type=int, default=0)
     parser.add_argument('--split', type=str, default='fixed')                      #【mask】：active、fixed、low、pl_random、pl_noise_random、active_train
     ...
     parser.add_argument("--yaml_path",type=str,default="D:/Python/pythonProject/LLMGNN-master/config.yaml")
     parser.add_argument("--no_val", type=int, default=0)                           #【无验证集】cora 1、
     ...
     parser.add_argument("--debug", type=int, default=0)                            #【若不为零，则表示debug和记录每次的结果文件】cora 1、
     parser.add_argument("--train_vs_val", type=float, default = 3)
     parser.add_argument("--total_budget", type=int, default = -1)                  #【预算，中等】cora 140、citeseer 240、
     ...
     parser.add_argument("--alpha", type=float, default = 0.1)                      #【loss比重系数】cora 0.4、citeseer 0.4
     parser.add_argument("--beta", type=float, default = 0.1)                       #【loss比重系数】cora 0.3、citeseer 0.3
     parser.add_argument("--gamma", type=float, default = 0.1)
     parser.add_argument("--ratio", type=float, default = 0.3)                      #【】cora 0.2、、citeseer 0.2
...
def get_command_line_args_cora():
     parser = argparse.ArgumentParser(description='LLM Graph')
     parser.add_argument('--dataset', default='cora', type=str)          #【数据集】：cora、pubmed、citeseer、arxiv
     parser.add_argument('--normalize', default=0, type=int)
     parser.add_argument('--epochs', type=int, default=22)               #【训练轮次】，cora 22、citeseer 16
     parser.add_argument('--early_stopping', type=int, default=10)
     parser.add_argument('--model_name', type=str, default='GCN')        #【被训练模型】可选：GCN、MLP、SAGE、GAT、LP、S_model、MLP2、BSAGE、AdjGCN、AdjSAGE
     parser.add_argument('--norm', type=str, default=None)
     parser.add_argument('--main_seed_num', type=int, default=5)         #【随机种子】，cora 5、citeseer 5
     parser.add_argument('--sweep_seed_num', type=int, default=5)
     parser.add_argument('--return_embeds', type=int, default=1)
     parser.add_argument('--lr', type=float, default=0.01)
     parser.add_argument('--weight_decay', type=float, default=5e-4)
     ...
     parser.add_argument('--lr_gamma', default=0.998, type=float)
     parser.add_argument('--data_format', type=str, default='sbert')                #【】
     parser.add_argument('--early_stop_start', type=int, default=150)               #【早停参数，防止过拟合】cora 150
     # parser.add_argument('--alpha', type=float, default=0.9)
     parser.add_argument('--low_label_test', type=int, default=0)
     parser.add_argument('--few_shot_test', type=int, default=0)
     ...
     parser.add_argument("--pl_noise", type=float, default=0)
     parser.add_argument("--yaml_path",type=str,default="D:/Python/pythonProject/LLMGNN-master/config.yaml")
     parser.add_argument("--no_val", type=int, default=1)                           #【无验证集】cora 1、
     parser.add_argument("--label_smoothing", type=float, default=0)
     parser.add_argument("--budget", type=int, default=20)
     ...
     parser.add_argument("--save_data", type=int, default=0)
     parser.add_argument("--max_part", type=int, default=7)
     parser.add_argument("--debug", type=int, default=1)                            #【若不为零，则表示debug和记录每次的结果文件】cora 1、
     parser.add_argument("--train_vs_val", type=float, default = 3)
     parser.add_argument("--total_budget", type=int, default = 140)                 #【预算，中等】cora 140、citeseer 240、
     parser.add_argument("--loss_type", type=str, default = 'ce')
     ...
     parser.add_argument("--filter_all_wrong_labels", type=int, default = 0)        #【】
     parser.add_argument("--oracle", type=float, default = 1.0)                     #【】
     parser.add_argument("--alpha", type=float, default = 0.33)                      #【loss比重系数】cora 0.33、citeseer 0.4
     parser.add_argument("--beta", type=float, default = 0.33)                       #【loss比重系数】cora 0.33、citeseer 0.3
     parser.add_argument("--gamma", type=float, default = 0.1)
     parser.add_argument("--ratio", type=float, default = 0.29)                      #【】cora 0.29、citeseer 0.2
     args = parser.parse_args()
     return args

def get_command_line_args_citeseer():
     parser = argparse.ArgumentParser(description='LLM Graph')
     parser.add_argument('--dataset', default='citeseer', type=str)      #【数据集】：cora、pubmed、citeseer、arxiv
     parser.add_argument('--normalize', default=0, type=int)
     parser.add_argument('--epochs', type=int, default=50)               #【训练轮次】，cora 22、citeseer 22
     parser.add_argument('--early_stopping', type=int, default=10)
     parser.add_argument('--model_name', type=str, default='GCN')        #【被训练模型】可选：GCN、MLP、SAGE、GAT、LP、S_model、MLP2、BSAGE、AdjGCN、AdjSAGE
     parser.add_argument('--norm', type=str, default=None)
     parser.add_argument('--main_seed_num', type=int, default=3)         #【随机种子】，cora 5、citeseer 5
     parser.add_argument('--sweep_seed_num', type=int, default=5)
     parser.add_argument('--return_embeds', type=int, default=1)
     ...
     parser.add_argument("--save_data", type=int, default=0)
     parser.add_argument("--max_part", type=int, default=7)
     parser.add_argument("--debug", type=int, default=1)                                    #【若不为零，则表示debug中和记录每次的结果文件】cora 1、
     parser.add_argument("--train_vs_val", type=float, default = 3)
     parser.add_argument("--total_budget", type=int, default = 240)                         #【预算，中等】cora 140、citeseer 240、
     parser.add_argument("--loss_type", type=str, default = 'ce')
     parser.add_argument("--second_filter", type=str, default = 'conf+density')             #二、【后过滤策略】weight 或其它选项：conf_only、density_only、conf+density、conf+entropy、conf+density+entropy
     parser.add_argument("--debug_gt_label", type=int, default = 0)                         #【】
     parser.add_argument("--train_stage", type=int, default = 1)
     parser.add_argument("--filter_all_wrong_labels", type=int, default = 0)                #【】
     parser.add_argument("--oracle", type=float, default = 1.0)                             #【】
     parser.add_argument("--alpha", type=float, default = 0.33)                              #【loss比重系数】citeseer 0.33、citeseer 0.33
     parser.add_argument("--beta", type=float, default = 0.33)                               #【loss比重系数】citeseer 0.33、citeseer 0.33
     parser.add_argument("--gamma", type=float, default = 0.1)
     parser.add_argument("--ratio", type=float, default = 0.54)                              #【】citeseer 0.29、citeseer 0.29
     args = parser.parse_args()
     return args

def get_command_line_args_pubmed():
     parser = argparse.ArgumentParser(description='LLM Graph')
     parser.add_argument('--dataset', default='pubmed', type=str)      #【数据集】：cora、pubmed、citeseer、arxiv
     parser.add_argument('--normalize', default=0, type=int)
     parser.add_argument('--epochs', type=int, default=30)               #【训练轮次】，cora 22、citeseer 22
     parser.add_argument('--early_stopping', type=int, default=10)
     parser.add_argument('--model_name', type=str, default='GCN')        #【被训练模型】可选：GCN、MLP、SAGE、GAT、LP、S_model、MLP2、BSAGE、AdjGCN、AdjSAGE
     parser.add_argument('--norm', type=str, default=None)
     parser.add_argument('--main_seed_num', type=int, default=3)         #【随机种子】，cora 5、citeseer 5
     ...
     parser.add_argument("--debug", type=int, default=1)                                    #【若不为零，则表示debug中和记录每次的结果文件】cora 1、
     parser.add_argument("--train_vs_val", type=float, default = 3)
     parser.add_argument("--total_budget", type=int, default = 120)                         #【预算，中等】cora 140、citeseer 240、
     parser.add_argument("--loss_type", type=str, default = 'ce')
     parser.add_argument("--second_filter", type=str, default = 'conf+density')             #二、【后过滤策略】weight 或其它选项：conf_only、density_only、conf+density、conf+entropy、conf+density+entropy
     parser.add_argument("--debug_gt_label", type=int, default = 0)                         #【】
     parser.add_argument("--train_stage", type=int, default = 1)
     ...
     parser.add_argument("--beta", type=float, default = 0.33)                               #【loss比重系数】citeseer 0.33、citeseer 0.33
     parser.add_argument("--gamma", type=float, default = 0.1)
     parser.add_argument("--ratio", type=float, default = 0.2)                              #【】citeseer 0.29、citeseer 0.29
     args = parser.parse_args()
     return args
