...
def generate_labeled_data(data, random_index, num_classes, labeled_per_class):
    if labeled_per_class<=0:
        return torch.tensor([], dtype=random_index.dtype), random_index
    labeled_set = {}
    label_Num = num_classes * labeled_per_class
    label_num_total = 0
    labeled_nodes = []
    ...
    for index_i in random_index:
        ...
        if class_i not in labeled_set:
            ...
            if labeled_set[class_i] >= labeled_per_class: continue
            labeled_set[class_i] += 1
            label_num_total += 1
            labeled_nodes.append(index_i)
...
def generate_labeled_pl_mask(data, no_val = 0, total_budget = 140, labeled_per_class = 3, seed = 0):
    seed_everything(seed)                       #多种随机数生成器
    num_classes = data.y.max().item() + 1
    total_num = data.x.shape[0]
    total_label_num = total_budget             #取之前precompute.py的结果处于中等的budget，比如cora140、citeseer120，其意义是什么？每类预生成的数量 - 也衡量模型的预算
...
def get_dataset(seeds, dataset, split, data_format, labeled_per_class, data_path, logit_path, random_noise = 0, no_val = 1, budget = 20, strategy = 'random', num_centers = 0, compensation = 0, save_data = 0, llm_strategy = 'none', max_part = 0, oracle_acc = 1, reliability_list = None, total_budget = -1, second_filter = 'none', train_stage = True, post_pro = False, filter_all_wrong_labels = False, alpha = 0.1, beta = 0.1, gamma = 0.1, ratio = 0.3, fixed_data = True):
    seed_num = len(seeds)
    ...
    for s in seeds:
        ...
        if split == 'fixed':
            ...
            l_mask, t_mask, val_mask, test_mask = generate_labeled_pl_mask(data, no_val, real_budget, labeled_per_class)
