...
def gen_labeled_data(data, random_index, num_classes, label_num_per_class = 3):
    labeled_set = {}
    label_Num = num_classes * label_num_per_class
    label_num_total = 0
    labeled_nodes = []
    # data_NumDict = {}  #数据集各类节点统计
    # for kk in data.y:
    #     class_i = kk.item()
    #     if class_i in data_NumDict:
    #         data_NumDict[class_i] += 1
    #     else:
    #         data_NumDict[class_i] = 1
    # print(data_NumDict)
    for index_i in random_index:
        class_i = data.y[index_i].item()
        if class_i not in labeled_set:
            labeled_set[class_i] = 1
            label_num_total += 1
            labeled_nodes.append(index_i)
        else:
            if labeled_set[class_i] >= label_num_per_class: continue
            labeled_set[class_i] += 1
            label_num_total += 1
            labeled_nodes.append(index_i)
        if label_num_total >= label_Num: break
        # print(label_num_total,': ', labeled_nodes)
    labeled_nodes = torch.tensor(labeled_nodes, dtype=random_index.dtype)
    # print(': ', labeled_nodes)
    remaining_nodes = random_index[~torch.isin(random_index, labeled_nodes)]
    return labeled_nodes, remaining_nodes

def generate_l_pl_mask(data, no_val = 0, total_budget = 140, seed = 0):
    seed_everything(seed)                       #多种随机数生成器
    num_classes = data.y.max().item() + 1
    total_num = data.x.shape[0]
    total_label_num = total_budget             #取之前precompute.py的结果处于中等的budget，比如cora140、citeseer120，其意义是什么？每类预生成的数量 - 也衡量模型的预算
    ...
    label_num_per_class = 3
    labeled_nodes, unlabeled_nodes = gen_labeled_data(data, random_index, num_classes, label_num_per_class)
    # print("label:", labeled_nodes, ", unlabel: ", unlabeled_nodes)
    train_num = train_num - num_classes * label_num_per_class
    ...
    # print("label:", labeled_nodes, ", unlabel: ", test_index)

    l_mask, t_mask, val_mask, test_mask = index_to_mask(labeled_nodes, total_node_number), index_to_mask(train_index, total_node_number), index_to_mask(val_index, total_node_number), index_to_mask(test_index, total_node_number)
    # print("label:", torch.sum(l_mask == True), ", unlabel: ", torch.sum(t_mask == True), torch.sum(val_mask == True), torch.sum(test_mask == True))
    return l_mask, t_mask, val_mask, test_mask
...
def get_dataset(seeds, dataset, split, data_format, data_path, logit_path, random_noise = 0, no_val = 1, budget = 20, strategy = 'random', num_centers = 0, compensation = 0, save_data = 0, llm_strategy = 'none', max_part = 0, oracle_acc = 1, reliability_list = None, total_budget = -1, second_filter = 'none', train_stage = True, post_pro = False, filter_all_wrong_labels = False, alpha = 0.1, beta = 0.1, gamma = 0.1, ratio = 0.3, fixed_data = True):
    ...
    for s in seeds:
        ...
        if split == 'fixed':
            ...
            l_mask, _, val_mask, test_mask = generate_l_pl_mask(data, no_val, real_budget)
            # import ipdb; ipdb.set_trace()
            ...
            train_set = l_mask|train_mask
            test_mask = ~train_set
            # print(torch.sum(train_mask == True),'\n',torch.sum(train_set == True))
            ...
            new_labeled_masks.append(l_mask)                           #保存每个seed下的划分的有标签集、无标签集
        ...
        ## start from 
    # import ipdb; ipdb.set_trace()
    data.labeled_masks = new_labeled_masks
